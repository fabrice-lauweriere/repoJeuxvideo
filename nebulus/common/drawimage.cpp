...
DrawImage::DrawImage(std::string filename)
{
  ImageRGBA_Ptr img;
  try {
    img = loadImage<ImageRGB>( filename.c_str() )->cast<ImageRGBA>();
    ForImage(img,i,j) { // fill up alpha with 255
      img->pixel(i,j)[3] = 255;
    }
  } catch (...) {
    img = loadImage<ImageRGBA>( filename.c_str() );
  }
  m_Tex             = new Tex2DRGBA( img->pixels() );
}
...
void DrawImage::drawSub(v2i screen_pos,v2i screen_size,v2i table_pos,v2i table_size)
{
  ...
  s_DrawImage.u_ImageSize.set( V2F((float)m_Tex->w(),(float)m_Tex->h()) );
  s_DrawImage.u_UVWH     .set( V4F(table_pos[0],table_pos[1],table_size[0],table_size[1]) );
  sq.render();
  s_DrawImage.end(); 
  glPopAttrib();
}
